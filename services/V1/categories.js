...
const fieldName = {
  keyField: 'id',
  listSortField: 'categoryName',
  dropDownField: 'categoryName'
};
...
async function getItem(context) {
  const result = await imports.db.categories.findOne({
    where: { 'ID': context.id },
  });
  return result;
}
...
async function getItemsList(context) {
  ...
  const tableNames = `categories WHERE isenabled = 1 AND isdeleted = 0`;
...
async function getItemsDropDown(context) {
  ...
  const selectQuery = `SELECT id,categoryName FROM `;
...
async function createItem(test) {
  try {
    const categoryObj = JSON.parse(JSON.stringify(test.category));
    console.log('=========>>>>DBAPI', categoryObj);
    const result = await imports.db.categories.create(categoryObj);
    return result;
  } catch (err) {
...
async function updateItem(test, id) {
  const categoryObj = test;
  const result = await imports.db.categories.update(categoryObj, { where: { 'ID': id } });
  return result;
}
